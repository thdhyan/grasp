...
def pos_out_of_radius(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("chair"),
    radius: float = 5.0,
) -> torch.Tensor:
    """Termination: asset position out of specified radius from origin.
    
    Terminates if the specified asset moves beyond a certain radius
    from the environment origin in the horizontal plane.
    
    Args:
        env: The RL environment instance.
        asset_cfg: Configuration for the asset to monitor.
        radius: Maximum allowed radius from origin in meters.
    ...
    # Get asset position relative to env origin
    asset_pos = asset.data.root_pos_w - env.scene.env_origins  # Shape (num_envs, 3)
    
    # Compute horizontal distance from origin
    horizontal_dist = torch.sqrt(asset_pos[:, 0]**2 + asset_pos[:, 1]**2)
    
    # Terminate if distance exceeds radius
    return horizontal_dist > radius
